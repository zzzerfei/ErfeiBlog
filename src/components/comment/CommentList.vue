...
<template>
  <div class="comment-list">
    <div v-if="!list" class="text-red">还没有评论喔！快来抢占沙发~~~</div>
    <div v-else class="top-title" >
      <span>可以在这里发表您的看法或建议</span>
      <span>共{{ numbers }} 条评论</span>
    </div>
    <div v-for="item in list" :key="item._id" class="item">
      ...
      <div v-for="e in item.other_comments" :key="e._id" class="item-other">
        ...
        <div class="item-comment">
          <div class="message">
            <el-button @click="showCommentModal(item._id, item.user, e.user)" size="small"
              >回复</el-button
            >
          </div>
        </div>
